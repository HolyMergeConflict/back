...
class TaskHistoryService:
    ...
    async def log_attempt(self, user: User, data: TaskHistoryCreate) -> TaskHistory:
        ...
        return await self._crud.create(history)

    async def get_user_history(self, current_user: User, target_user_id: int) -> list[TaskHistory]:
        self._ensure_own_data(current_user, target_user_id)
        self.logger.info(f'Fetching all history for user_id={target_user_id}')
        return await self._crud.get_all(user_id=target_user_id)

    async def get_user_history_by_status(self, current_user: User, status: TaskSolutionStatusEnum) -> list[TaskHistory]:
        self.logger.info(f'Fetching history with status={status} for user_id={current_user.id}')
        return await self._crud.get_all(user_id=current_user.id, status=status)

    async def get_task_history_for_user(self, current_user: User, task_id: int) -> list[TaskHistory]:
        self.logger.info(f'Fetching history for user_id={current_user.id}, task_id={task_id}')
        return await self._crud.get_by_user_and_task(user_id=current_user.id, task_id=task_id)

    async def get_task_history_in_period(self, current_user: User, start: datetime, end: datetime) -> list[TaskHistory]:
        self.logger.info(f'Fetching history in time range: {start}–{end} for user_id={current_user.id}')
        return await self._crud.get_in_time_range(current_user.id, start, end)

    async def get_latest_result(self, current_user: User, task_id: int) -> TaskHistory | None:
        self.logger.info(f'Fetching latest result for user_id={current_user.id}, task_id={task_id}')
        return await self._crud.get_latest_attempt(user_id=current_user.id, task_id=task_id)
